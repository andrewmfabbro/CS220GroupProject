...
void add(std::string title) { //add a new title alphabetically
    if(myStore->find(title)){
        std::cout << "Book Already Exists In BookStore!" << std::endl;
        modify(title);
        return;
    }
    int newWant;
...
void modify(std::string title) { //modify the want values
    ...
    if(bookToModify != nullptr) {
        int newWant;
        std::cout << "Current Have: " << bookToModify->getHaveValue() << " Current Want: "
                  << bookToModify->getNeedValue() << std::endl;
...
//purchase more books
void order(){
    std::cout << "Ordering new titles. Shipping Manifest stored in orders.txt" << std::endl;
    myStore->makeOrder();
}

//read from file, then add books from file to store
void delivery(){
    std::cout << "Delivery Received." << std::endl;
    myStore->takeDelivery();
}

//return books, make have value match want value
void invoice(){
    std::cout << "Returns sent, Returned items were recorded in returns.txt" << std::endl;
    myStore->returnBooks();
}

void quit(){
    myStore->printToFile("testFile.txt");
    interface = false;
}
...
int main() {
    myStore ->loadBookFile("testFile.txt");
    while(interface){
        Interface();
    }
